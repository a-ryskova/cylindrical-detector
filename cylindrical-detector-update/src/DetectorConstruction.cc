...
void DetectorConstruction::ConstructSDandField() {
    // Создание и регистрация чувствительного детектора для цилиндра
    MySensitiveDetector* cylinderSD = new MySensitiveDetector("CylinderSD");
    G4SDManager::GetSDMpointer()->AddNewDetector(cylinderSD);
    cylinderLogic->SetSensitiveDetector(cylinderSD);
    
    G4cout << "Чувствительный детектор присоединен к цилиндру" << G4endl;
    
    // Также добавляем сенситивный детектор к свинцовому слою для регистрации взаимодействий в нем
    MySensitiveDetector* leadSD = new MySensitiveDetector("LeadSD");
    G4SDManager::GetSDMpointer()->AddNewDetector(leadSD);
    leadLayerLogic->SetSensitiveDetector(leadSD);
    
    G4cout << "Чувствительный детектор присоединен к свинцовому слою" << G4endl;
}
